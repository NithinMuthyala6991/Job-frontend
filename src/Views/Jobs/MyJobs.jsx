...
const MyJobs = () => {
  ...
  const getJobsList = async () => {
    try {
      const res = await http.getApi("/get-my-jobs");
      console.log(res)
      if(res.data.success){
    ...
    <div className="container">
      <div className="row mb-5 justify-content-center">
        <div className="col-md-7 text-center">
         {data.length >0 && <h2 className="section-title mb-2">You have posted {data.length > 0 ? data?.length : ''} jobs</h2>}
        </div>
      </div>
      <ul className="job-listings mb-5">
        {
          ...
          }) : <h1>You haven't posted jobs yet</h1>
        }
